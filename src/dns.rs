        ...
        match socket.recv_from(&mut buf).await {
            Ok((len, peer)) => {
                ...
                tokio::spawn(async move {
                    let resp = process_dns_wire(&req_wire, &state_ref, "UDP").await;
                    if !resp.is_empty() {
    ...
    loop {
        match listener.accept().await {
            Ok((stream, _peer)) => {
                ...
                let state_ref = state.clone();
                tokio::spawn(async move {
                    handle_length_prefixed_stream(stream, state_ref, "TCP").await;
                });
            }
            Err(e) => {
...
pub async fn handle_length_prefixed_stream<S>(mut stream: S, state: AppState, protocol: &'static str)
where
    S: AsyncRead + AsyncWrite + Unpin,
{
    ...
    while stream.read_exact(&mut len_buf).await.is_ok() {
        ...
        let resp_wire = process_dns_wire(&req_buf, &state, protocol).await;
